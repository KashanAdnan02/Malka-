...
export default function RecordForm({
  ...
  const fetchRates = async () => {
    const data = await fetch(
      "https://beta-restapi.sarmaaya.pk/api/commodities/gold-22k",
    );
    console.log(data)
  };
  fetchRates()
            ...
            <Dropdown
              ...
              renderOption={(opt, isSelected) => (
                <div className="flex items-center gap-3 py-0.5">
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${isSelected ? "bg-amber-100 text-amber-600" : "bg-slate-100 text-slate-500"}`}
                  >
                    <ShopIcon />
                  </div>
