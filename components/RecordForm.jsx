...
const RATI_OPTIONS = [18, 20, 22, 23, 24];
...
function SectionLabel({ icon, children }) {
  return (
    <div className="flex items-center gap-3 mb-5">
      ...
      <p className="text-xs font-bold text-slate-400 uppercase tracking-widest whitespace-nowrap">{children}</p>
...
function Dropdown({ label, value, onChange, options, placeholder = "Select…", renderOption, renderTrigger }) {
  ...
  useEffect(() => {
    const handler = (e) => { if (ref.current && !ref.current.contains(e.target)) closeMenu(); };
    document.addEventListener("mousedown", handler);
    return () => document.removeEventListener("mousedown", handler);
  }, []);

  const openMenu = () => { setOpen(true); requestAnimationFrame(() => setAnimIn(true)); };
  const closeMenu = () => { setAnimIn(false); setTimeout(() => setOpen(false), 200); };
  ...
  return (
    <div className="flex flex-col gap-1.5" ref={ref}>
      ...
      <div className="relative">
        ...
        {open && (
          <div
            className="absolute z-50 mt-2 w-full bg-white border border-slate-100 rounded-2xl overflow-hidden"
            style={{
              ...
              {options.map((opt) => {
                ...
                return (
                  <li key={val}>
                    <button
                      type="button"
                      onClick={() => { onChange(val); closeMenu(); }}
...
function ShopInfoCard({ shop }) {
  ...
  return (
    ...
    >
      {[
        { icon: <ShopIcon />, label: "Shop", value: shop.shopName },
        { icon: <UserIcon />, label: "Contact", value: shop.name },
        { icon: <PhoneIcon />, label: "Phone", value: shop.number },
      ].map((item) => (
        <div key={item.label}>
...
export default function RecordForm({
  ...
  const selectedShop = shops.find((s) => (s._id || s.id) === form.shopId);
      ...
      <div className="p-6 space-y-8">
        ...
        <AnimatedSection delay={50}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <InputField label="Item Name" name="item" value={form.item} onChange={handleChange} placeholder="e.g. Bunda, Kangan..." />
            <InputField label="Weight (grams)" name="grams" placeholder="e.g. 3.45" value={form.grams} onChange={handleChange} />
            <InputField label="Price (₨)" name="price" placeholder="e.g. 50000" value={form.price} onChange={handleChange} />
            <div className="sm:col-span-2">
              <RatiPicker value={form.rati} onChange={(val) => setForm((p) => ({ ...p, rati: val }))} />
            </div>
          </div>
        </AnimatedSection>
            ...
            <Dropdown
              ...
              renderOption={(opt, isSelected) => (
                <div className="flex items-center gap-3 py-0.5">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${isSelected ? "bg-amber-100 text-amber-600" : "bg-slate-100 text-slate-500"}`}>
                    <ShopIcon />
                  </div>
                  <div className="min-w-0">
                    <p className={`text-sm font-semibold truncate ${isSelected ? "text-amber-800" : "text-slate-800"}`}>{opt.label}</p>
                    <p className="text-xs text-slate-400 truncate">{opt.sub} · {opt.number}</p>
                  </div>
                </div>
              )}
              renderTrigger={(opt) => (
                <span className="flex items-center gap-2 min-w-0">
                  <span className="text-amber-500 shrink-0"><ShopIcon /></span>
                  <span className="font-semibold truncate">{opt.label}</span>
                  {opt.sub && <span className="text-slate-400 font-normal text-xs truncate">· {opt.sub}</span>}
                </span>
              )}
            />
